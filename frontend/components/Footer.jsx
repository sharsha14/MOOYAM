...
const Footer = () => {
    ...
    const linkSections = [
        {
            title: "COLLECTIONS",
            links: [
                { text: "SkinCare", path: '/shop?category=SkinCare', icon: null },
                { text: "HairCare", path: '/shop?category=HairCare', icon: null },
                { text: "Makeup", path: '/shop?category=Makeup', icon: null },
                { text: "New Arrivals", path: '/shop?sort=newest', icon: null },
            ]
        },
        {
            title: "COMPANY",
        ...
        },
        {
            title: "CONTACT",
            links: [
                { text: "+91 98765 43210", path: '/', icon: PhoneIcon },
                { text: "contact@lumiere.com", path: '/', icon: MailIcon },
                { text: "123 Beverly Blvd, Los Angeles, CA", path: '/', icon: MapPinIcon }
            ]
        }
    ];
    ...
    return (
        <footer id="contact" className="bg-[#F9F3F1] text-[#2C2C2C] mt-20 border-t border-[#D4A398]/30">
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-16 border-b border-[#D4A398]/30">
                    <div>
                        <Link href="/" className="text-4xl font-bold font-serif tracking-widest uppercase text-[#D4A398]">
                            MOO<span className="text-[#2C2C2C] font-light italic">YAM</span><span className="text-[#2C2C2C] text-4xl">.</span>
                        </Link>
                        <p className="max-w-[400px] mt-6 text-sm leading-relaxed text-[#2C2C2C]/80 font-light tracking-wide">
                            Discover the essence of beauty with MOOYAM. We bring you an exclusive collection of luxury skincare, nourishing haircare, and premium makeup, crafted for elegance and glow.
                        </p>
                        <div className="flex items-center gap-4 mt-8">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} className="flex items-center justify-center w-10 h-10 bg-white hover:bg-[#D4A398] hover:text-white transition-all duration-300 rounded-full group shadow-sm border border-[#D4A398]/30">
                                    <div className="group-hover:stroke-white stroke-[#D4A398]">
                                        <item.icon />
                                    </div>
                                </Link>
                            ))}
                    ...
                    <div className="flex flex-wrap justify-between w-full md:w-[50%] gap-8 text-sm">
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-serif font-medium text-[#D4A398] mb-5 tracking-widest text-xs uppercase">{section.title}</h3>
                                <ul className="space-y-3">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-3 group">
                                            {link.icon && <span className="text-[#D4A398]"><link.icon /></span>}
                                            <Link href={link.path} className="text-[#2C2C2C] hover:text-[#D4A398] transition-colors duration-300">{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <div className="py-6 flex flex-col sm:flex-row justify-between items-center text-xs text-[#2C2C2C]/60 font-light tracking-wide">
                    <p>Copyright 2026 © MOOYAM. All Rights Reserved.</p>
                    <p>Crafted for Beauty.</p>
                </div>
            </div>
        </footer>
    );
